...
| **Sweet spot** | Low | Low | Good generalization |
| **Overfitting** | Very low | Rising | Memorizing, not learning |
...
- This structure is what makes generalization possible

> Deep learning works because it learns parametric curves that approximate these data manifolds.
...
- **Temporal ordering**: For time-series, *never* shuffle — validate on future data only
...
- Try: lower learning rate, larger batch size
...
## The generalization recipe

```{mermaid}
%%| fig-width: 8
flowchart TD
    A[Start: build a model that overfits] --> B[Add regularization / dropout]
    B --> C[Tune hyperparameters on validation set]
    C --> D{Validation improving?}
    D -->|Yes| C
    D -->|No| E[Evaluate on test set once]
```

::: notes
- Step 1 is crucial: if you can't overfit, regularization won't help
- Iterate between capacity and regularization
- Only touch the test set at the very end
:::
