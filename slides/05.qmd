...
- **NOTEBOOK DEMO**: Switch to notebook cells 10-12. Run the noise channels experiment and show the validation accuracy plot. Ask: "Why can't the model just ignore the noise?"
:::
...
- **NOTEBOOK DEMO**: Switch to notebook cells 44-48. Run the IMDB baseline (16-unit) model, then the larger (512-unit) model. Plot both to show how the bigger model overfits much worse. (Pre-run these if short on time.)
:::
...
- **NOTEBOOK DEMO**: Switch to notebook cells 54-55. Run the dropout model on IMDB and plot against the original baseline from the previous demo. The validation curve tightens up — dropout fixes the overfitting.
:::
